...
import { costSMSCmncmccUpdate } from '../../../../constants/contextpath/admin/costmanagement/CostManagement';
function CostSmscMncmccEdit() {
    const navigate = useNavigate();
    const dispatch = useDispatch();
    const location = useLocation(); // Access location to get state data

    // Set the page name in the Redux store
    const pagename = { pagename: 'Cost SMSC MCC & MNC Edit' };
    ...
    const initialData = location.state?.costsmscmncmcc; // Assuming data is passed as state
    const [formData, setFormData] = useState({
        cost_id: initialData?.cost_id || '',
        cost: initialData?.cost || '',
        mcc: initialData?.mcc || '',
        mnc: initialData?.mnc || '',
        smscid: initialData?.smscid || '',
    ...
    const handleSubmit = (e) => {
        ...
        connectAPIViaPost(formData,costSMSCmncmccUpdate )
            .then((response) => {
                Setsuccess('Cost SMSC MCC & MNC updated successfully');
                Seterror('');
            })
            .catch((error) => {
                if (error.response) {
    ...
    return (
        <div className='w-[100%] h-auto min-h-screen admin-inner-body1 pb-10'>
            <div className="md:pl-[33px] pl-[20px] md:pt-[10px] pt-[10px] mb-[10px] flex">
                <p className="text-admin-color2">Cost Management &gt; Cost SMSC MCC & MNC Edit</p>
            </div>
            <div className="md:px-[23px] px-[8px]">
                <div className="admin-inner-body2 md:px-[30px] px-[10px] py-[20px] rounded-xl">
                    <main>
                        <div className='flex justify-between items-center mb-4'>
                            <h1 className="text-xl font-semibold admin-table-text text-table-text">Cost SMSC MCC & MNC Edit</h1>
                        </div>
                        {error && (
                        ...
                        <form onSubmit={handleSubmit}>
                            <div className="-mx-3 md:flex md:mb-4">
                                ...
                                <div className="md:w-1/2 px-3 mb-6 md:mb-0 group relative">
                                    ...
                                    <input
                                        ...
                                        onChange={handleChange}
                                        className="relative appearance-none block w-full bg-white text-table-text border border-red rounded py-2 px-2 mb-3 cursor-not-allowed"
                                        id="smscid"
                                        placeholder="smscid"
                                        disabled
                                    />
                            ...
                            <div className="-mx-3 md:flex md:mb-4">
                                <div className="md:w-1/2 px-3 mb-6 md:mb-0 group relative">
                                    ...
                                    <select
                                        ...
                                        onChange={handleChange}
                                        className="appearance-none block w-full bg-white text-grey-darker border border-gray-400 rounded py-2 px-2 mb-3 cursor-not-allowed"
                                        disabled
                                    >
                            ...
                            <Button type="submit">Update Cost Smsc Mnc mcc </Button>
                        </form>
                    </main>
                </div>
            </div>
...
export default CostSmscMncmccEdit;
