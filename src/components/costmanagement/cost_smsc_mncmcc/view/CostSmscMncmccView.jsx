...
function CostSmscMncmccView() {
    const location = useLocation();
    const dispatch = useDispatch();
    const pagename = { pagename: 'Cost Smsc Mnc mcc View' };  // Update the page name if needed
    ...
    const [formData, setFormData] = useState({
        ...
        mcc: initialData?.mcc || '',
        mnc: initialData?.mnc || '',
        smscid: initialData?.smscid || '', // Changed from carriername to smscid
    ...
    return (
        <div className='w-full h-auto min-h-screen admin-inner-body1 pb-10 bg-gray-100'>
            <div className="md:pl-[33px] pl-[20px] md:pt-[10px] pt-[10px] mb-[10px] flex">
                <p className="text-admin-color2">Cost Management &gt; Cost SMSC MCC & MNC View</p>
            </div>
            <div className="md:px-[23px] px-[8px]">
                <div className="admin-inner-body2 md:px-[30px] px-[10px] py-[20px] rounded-xl shadow-md">
                    <main>
                        <div className='flex justify-between items-center mb-4'>
                            <h1 className="text-xl font-semibold admin-table-text">Cost SMSC MCC & MNC View</h1>
                        </div>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 admin-table-text">
                                    ...
                                    <div className="p-4 border rounded-lg shadow-sm">
                                        <h2 className="text-xl font-semibold admin-table-text mb-4 flex items-center">
                                            <FaDollarSign className="mr-2 admin-table-text" /> Cost SMSC MCC & MNC Information
                                        </h2>
                                        <p className="mb-2"><strong>Cost:</strong> {initialData.cost || 'N/A'}</p>
                                        <p className="mb-2"><strong>SMSC ID:</strong> {initialData.smscid || 'N/A'}</p>
...
export default CostSmscMncmccView;
