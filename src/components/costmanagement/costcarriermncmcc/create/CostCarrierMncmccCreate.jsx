...
import { costCarrierMncmccCreate } from '../../../../constants/contextpath/admin/costmanagement/CostManagement';
function CostCarrierMncmccCreate() {
    const navigate = useNavigate();
    const dispatch = useDispatch();

    // Set the page name in the Redux store
    const pagename = { pagename: 'Cost Carrier MCC & MNC Create' };
    ...
    const handleChange = (e) => {
        ...
        // If MCC is selected, filter MNCs based on selected MCC
        if (name === 'mcc') {
    ...
    const handleSubmit = (e) => {
        ...
        connectAPIViaPost(formData,costCarrierMncmccCreate)
            .then((response) => {
                Setsuccess('Cost Carrier created successfully');
                Seterror('');
            })
            .catch((error) => {
                if (error.response) {
    ...
    return (
        <div className='w-[100%] h-auto min-h-screen admin-inner-body1 pb-10'>
            <div className="md:pl-[33px] pl-[20px] md:pt-[10px] pt-[10px] mb-[10px] flex">
                <p className="text-admin-color2">Cost Management &gt; Cost Carrier MCC & MNC Create</p>
            </div>
            <div className="md:px-[23px] px-[8px]">
                <div className="admin-inner-body2 md:px-[30px] px-[10px] py-[20px] rounded-xl">
                    <main>
                        <div className='flex justify-between items-center mb-4'>
                            <h1 className="text-xl font-semibold admin-table-text text-table-text">Cost Carrier MCC & MNC Create</h1>
                        </div>
                        {error && (
                        ...
                        <form onSubmit={handleSubmit}>
                            ...
                            <div className="">
                                <Button >Create Cost Carrier Mncmcc</Button>
                            </div>
                        </form>
                    </main>
                </div>
...
export default CostCarrierMncmccCreate;
