...
function ErrorcodeSmsServiceProviderCreate() {
    const dispatch = useDispatch();

    // Set the page name in the Redux store
    const pagename = { pagename: 'Errorcode Sms Service Provider Create' };
    dispatch(pagenamechange(pagename));

    const [error, Seterror] = useState("");
    const [success, Setsuccess] = useState("");
    const [PlatformErrorCode, setPlatformErrorCode] = useState();
    const [SmsServiceProviderall, setSmsServiceProviderall] = useState();
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                ...
                setSmsServiceProviderall(response.data.data);
            } catch (error) {
                console.error('Error fetching account list:', error);
            }
        };
    ...
    const handleSubmit = (e) => {
        ...
        connectAPIViaPost(formData, errorcodeSmsServiceProviderSave)
            .then((response) => {
                Setsuccess('Errorcode Sms Service Provider  Created successfully');
                Seterror('');
            })
            .catch((error) => {
                if (error.response) {
    ...
    return (
        <div className='w-[100%] h-auto min-h-screen admin-inner-body1 pb-10'>
            <div className="md:pl-[33px] pl-[20px] md:pt-[10px] pt-[10px] mb-[10px] flex">
                <p className="text-admin-color2">Errorcode Management &gt; Errorcode Sms Service Provider Create</p>
            </div>
                ...
                <div className="admin-inner-body2 md:px-[30px] px-[10px] py-[20px] rounded-xl">
                    <main>
                        <div className='flex justify-between items-center mb-4'>
                            <div>
                                <h1 className="text-xl font-semibold admin-table-text text-table-text">Errorcode Sms Service Provider Create</h1>
                            </div>
                        </div>
                        {error && (
                        ...
                        <form onSubmit={handleSubmit}>
                            ...
                            <div className="md:w-1/2 px-3">
                                    ...
                                    >
                                        ...
                                        {SmsServiceProviderall &&
                                            SmsServiceProviderall.map((SmsServiceProviderall) => (
                                                <option key={SmsServiceProviderall.smsserviceprovidername} value={SmsServiceProviderall.smsserviceprovidername}>
                                                    {SmsServiceProviderall.smsserviceprovidername}
                                                </option>
                                            ))}
                                    </select>
                                </div>
...
export default ErrorcodeSmsServiceProviderCreate;
