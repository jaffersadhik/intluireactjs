...
import { ipAccountUpdate } from '../../../../constants/contextpath/admin/ipmanagement/IpManagement';
function IpAccountEdit() {
  const location = useLocation();
  const ip = location.state?.IpAccount;

  const dispatch = useDispatch();
  const pagename = { pagename: 'Ip Account Edit' };
  ...
  const [formData, setFormData] = useState({
    ...
    ip: ip?.ip || '',
    ipaccount_id : ip?.ipaccount_id || '',
  });

  console.log(formData.accountname,"accountname")
  useEffect(() => {
  ...
  const handleSubmit = (e) => {
    ...
    connectAPIViaPost(formData, ipAccountUpdate)
      .then((response) => {
  ...
  return (
    <div className='w-[100%] h-auto min-h-screen admin-inner-body1 pb-10'>
      <div className="md:pl-[33px] pl-[20px] md:pt-[10px] pt-[10px] mb-[10px] flex">
        <p className="text-admin-color2">Ip Management &gt; Ip Account Edit</p>
      </div>
      <div className="md:px-[23px] px-[8px]">
        <div className="admin-inner-body2 md:px-[30px] px-[10px] py-[20px] rounded-xl">
          <main>
            <div className='flex justify-between items-center mb-4'>
              <h1 className="text-xl font-semibold admin-table-text text-table-text">Ip Account Edit</h1>
            ...
            <form onSubmit={handleSubmit} className="admin-inner-body2 rounded pt-2 pb-8 mb-4 flex flex-col my-2">
                    ...
                    {Account &&
                      Account.map((Account) => (
                        <option key={Account.username} value={Account.username}>
                          {decodePassword(Account.username)}
                        </option>
                      ))}
                  </select>
                </div>
              ...
              <div className="">
                <Button>Update Ip Account</Button>
