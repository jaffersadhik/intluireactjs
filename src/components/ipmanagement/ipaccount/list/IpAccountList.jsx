...
import { ipAccountGetAll } from '../../../../constants/contextpath/admin/ipmanagement/IpManagement';
function IpAccountList() {
    const navigate = useNavigate();
    const dispatch = useDispatch();
    const pagename = { pagename: 'Ip Account List' };
    ...
    const fetchData = async (currentPage) => {
        try {
            const response = await connectAPIViaGet(`${ipAccountGetAll}?page=${currentPage}&page_size=${pageSize}&search=${searchTerm}`);
    ...
    return (
        <div className='w-[100%] h-auto min-h-screen admin-inner-body1 pb-10'>
            <div className="md:pl-[33px] pl-[20px] md:pt-[10px] pt-[10px] mb-[10px] flex">
                <p className="text-admin-color2">Ip Management &gt; Ip Account List </p>
            </div>
                ...
                <div className="admin-inner-body2 md:px-[30px] px-[10px] py-[20px] rounded-xl">
                    <main>
                        <div className='flex justify-between items-center mb-4'>
                            <div className="">
                                <h1 className="md:text-xl text-[15px] font-semibold admin-table-text text-table-text">Ip Account List </h1>
                            </div>
                            <div className='flex'>
                                <button className="flex items-center uppercase gap-2 bg-admin-color2 p-2 rounded-md text-white" onClick={() => navigate('/ip/ipaccountcreate')}>
                                Ip Account  <IoMdAddCircleOutline />
                                </button>
                            </div>
                        </div>
                                        ...
                                        data.map((item) => (
                                            <tr key={item.customer_id} className="admin-inner-body2 border-b border-[#6b72808b]">
                                                ...
                                                <td className="px-6 py-4 gap-2 flex">
                                                    <button onClick={() => navigate('/ip/ipaccountview', { state: { IpAccount: item } })} className="px-4 py-2 text-xs font-bold uppercase rounded-full shadow-sm text-indigo-800 bg-indigo-100 hover:bg-indigo-200">
                                                        View
                                                    </button>

                                                    <button onClick={() => navigate('/ip/ipaccountedit', { state: { IpAccount: item } })}
