...
import { priceAccountUpdate } from '../../../../constants/contextpath/admin/pricemanagement/PriceManagement';
function PriceAccountEdit() {  // Updated component name
    const navigate = useNavigate();
    const dispatch = useDispatch();
    const location = useLocation();
    const PriceAccount = location.state.PriceAccount;  // Changed costsmscview to PriceAccount

    const pagename = { pagename: 'Price Account Edit' };  // Updated page name
    ...
    const handleSubmit = (e) => {
        ...
        connectAPIViaPost(updatedFormData,priceAccountUpdate)  // Updated the API URL
            .then((response) => {
                Setsuccess('Price account updated successfully');
                Seterror('');
            })
            .catch((error) => {
                if (error.response) {
    ...
    return (
        <div className='w-[100%] h-auto min-h-screen admin-inner-body1 pb-10'>
            <div className="md:pl-[33px] pl-[20px] md:pt-[10px] pt-[10px] mb-[10px] flex">
                <p className="text-admin-color2">Price Management &gt; Price Account Edit</p> {/* Updated breadcrumb */}
            </div>
                ...
                <div className="admin-inner-body2 md:px-[30px] px-[10px] py-[20px] rounded-xl">
                    <main>
                        <div className='flex justify-between items-center mb-4'>
                            <div>
                                <h1 className="text-xl font-semibold admin-table-text text-table-text">Price Account Edit</h1>  {/* Updated title */}
                            </div>
                        </div>
                        {error && (
                        ...
                        <form onSubmit={handleSubmit}>
                            ...
                            <div className="form-group">
                                <Button >Update Price Account</Button>  {/* Updated button label */}
                            </div>
                        </form>
                    </main>
                </div>
...
export default PriceAccountEdit;  // Exporting the updated component
