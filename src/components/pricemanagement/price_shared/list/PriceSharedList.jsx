...
import { priceSharedList } from '../../../../constants/contextpath/admin/pricemanagement/PriceManagement';
function PriceSharedList() {  // Updated component name
    const navigate = useNavigate();
    const dispatch = useDispatch();
    const pagename = { pagename: 'Price Shared List' };  // Update the page name if needed
    ...
    const fetchData = async (currentPage) => {
        try {
            const response = await connectAPIViaGet(`${priceSharedList}?page=${currentPage}&page_size=${pageSize}&search=${searchTerm}`);
    ...
    return (
        <div className='w-[100%] h-auto min-h-screen admin-inner-body1 pb-10'>
            <div className="md:pl-[33px] pl-[20px] md:pt-[10px] pt-[10px] mb-[10px] flex">
                <p className="text-admin-color2">Price Management &gt; Price Shared List </p>  {/* Update breadcrumb if needed */}
            </div>
                ...
                <div className="admin-inner-body2 md:px-[30px] px-[10px] py-[20px] rounded-xl">
                    <main>
                        <div className='flex justify-between items-center mb-4'>
                            <div className="">
                                <h1 className="md:text-xl font-semibold admin-table-text text-table-text">Price Shared List </h1>  {/* Update header if needed */}
                            </div>
                            <div className='flex'>
                                <button className="flex items-center uppercase gap-2 bg-admin-color2 p-2 rounded-md text-white" onClick={() => navigate('/price/pricesharedcreate')}>
                                Price Shared 
                                        ...
                                        data.map((item) => (
                                            <tr key={item.customer_id} className="admin-inner-body2 border-b border-[#6b72808b]">
                                                ...
                                                <td className="px-6 py-4 gap-2 flex">
                                                    <button onClick={() => navigate('/price/pricesharedview', { state: { PriceShared: item } })} className="px-4 py-2 text-xs font-bold uppercase rounded-full shadow-sm text-indigo-800 bg-indigo-100 hover:bg-indigo-200">
                                                        View
                                                    </button>

                                                    <button onClick={() => navigate('/price/pricesharededit', { state: { PriceShared: item } })}
...
export default PriceSharedList;  // Updated export statement
