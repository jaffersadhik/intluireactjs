...
import { routeCustomerMncmccEdit } from '../../../../constants/contextpath/admin/routemanagement/RouteManagement';
function RouteCustomerMncMccEdit() {
    const dispatch = useDispatch();
    const location = useLocation();

    const RouteCustomer = location.state?.RouteCustomer;

    // Set the page name in the Redux store
    const pagename = { pagename: 'Route Customer Mnc Mcc Edit' };
    ...
    const [formData, setFormData] = useState({
        routename: RouteCustomer?.routename || '',
        smscid: RouteCustomer?.smscid || '',
        dial_in_code : RouteCustomer?.dial_in_code || '',
        customername : RouteCustomer?.customername || '',
        weight : RouteCustomer?.weight || '',
        routemapping_id : RouteCustomer?. routemapping_id|| '',
        mcc:  RouteCustomer?. mcc|| '',
        mnc:  RouteCustomer?. mnc|| '',
      });
    ...
    const handleSubmit = (e) => {
        ...
        connectAPIViaPost(formData, routeCustomerMncmccEdit)
            .then((response) => {
                Setsuccess('Route customer Mnc Mcc Updated successfully');
                Seterror('');
            })
            .catch((error) => {
                if (error.response) {
    ...
    return (
        <div className='w-[100%] h-auto min-h-screen admin-inner-body1 pb-10'>
            <div className="md:pl-[33px] pl-[20px] md:pt-[10px] pt-[10px] mb-[10px] flex">
                <p className="text-admin-color2">Route Management &gt; Route Customer Mnc Mcc Edit</p>
            </div>
                ...
                <div className="admin-inner-body2 md:px-[30px] px-[10px] py-[20px] rounded-xl">
                    <main>
                        <div className='flex justify-between items-center mb-4'>
                            <div>
                                <h1 className="text-xl font-semibold admin-table-text text-table-text">Route Customer Mnc Mcc Edit</h1>
                            </div>
                        </div>
                        {error && (
...
export default RouteCustomerMncMccEdit;
