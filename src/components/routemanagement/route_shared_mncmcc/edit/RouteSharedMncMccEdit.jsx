...
import { routeSharedMncmccEdit } from '../../../../constants/contextpath/admin/routemanagement/RouteManagement';
function RouteSharedMncMccEdit() {
    const dispatch = useDispatch();
    const location = useLocation();
    const RouteShared = location.state?.RouteShared;
    // Set the page name in the Redux store
    const pagename = { pagename: 'Route Shared Edit' };
    ...
    const [formData, setFormData] = useState({
        ...
        routename: RouteShared?.routename || '',
        smscid: RouteShared?.smscid || '',
        dial_in_code : RouteShared?.dial_in_code || '',
        weight : RouteShared?.weight || '',
        routemapping_id : RouteShared?. routemapping_id|| '',
        mcc : RouteShared?.mcc || '',
        mnc : RouteShared?. mnc|| '',
    ...
    const handleSubmit = (e) => {
        ...
        connectAPIViaPost(formData,routeSharedMncmccEdit)
            .then((response) => {
                Setsuccess('Route Shared Mnc Mcc Update successfully');
                Seterror('');
            })
            .catch((error) => {
                if (error.response) {
    ...
    return (
        <div className='w-[100%] h-auto min-h-screen admin-inner-body1 pb-10'>
            <div className="md:pl-[33px] pl-[20px] md:pt-[10px] pt-[10px] mb-[10px] flex">
                <p className="text-admin-color2">Route Management &gt; Route Shared Edit</p>
            </div>
                ...
                <div className="admin-inner-body2 md:px-[30px] px-[10px] py-[20px] rounded-xl">
                    <main>
                        <div className='flex justify-between items-center mb-4'>
                            <div>
                                <h1 className="text-xl font-semibold admin-table-text text-table-text">Route Shared Edit</h1>
                            </div>
                        </div>
                        {error && (
                        ...
                        <form onSubmit={handleSubmit}>
                            ...
                            <div className="form-group">
                                <Button type="submit">Create Route Shared Mnc Mcc</Button>
                            </div>
                        </form>
                    </main>
                </div>
...
export default RouteSharedMncMccEdit;
