...
import { senderIdSharedUpdate } from '../../../../constants/contextpath/admin/senderidmanagement/SenderidManagement';
function SenderidsharedEdit() {
  const navigate = useNavigate();
  const location = useLocation();
  const senderid = location.state?.senderidshared;

  const dispatch = useDispatch();
  const pagename = { pagename: 'Senderid shared Edit' };
  ...
  const [formData, setFormData] = useState({
    senderid_id : senderid?.senderid_id || '',
  ...
  const handleSubmit = (e) => {
    ...
    connectAPIViaPost(formData, senderIdSharedUpdate)
      .then((response) => {
  ...
  return (
    <div className='w-[100%] h-auto min-h-screen admin-inner-body1 pb-10'>
      <div className="md:pl-[33px] pl-[20px] md:pt-[10px] pt-[10px] mb-[10px] flex">
        <p className="text-admin-color2">Senderid Management &gt; Senderid Account Edit</p>
      </div>
      <div className="md:px-[23px] px-[8px]">
        <div className="admin-inner-body2 md:px-[30px] px-[10px] py-[20px] rounded-xl">
          <main>
            <div className='flex justify-between items-center mb-4'>
              <h1 className="text-xl font-semibold admin-table-text text-table-text">Senderid Account Edit</h1>
            ...
            <form onSubmit={handleSubmit} className="admin-inner-body2 rounded pt-2 pb-8 mb-4 flex flex-col my-2">
              ...
              <div className="">
                <Button>Senderid shared Update</Button>
...
export default SenderidsharedEdit;
