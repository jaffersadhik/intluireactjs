...
import { dataCenterSave } from '../../../../constants/contextpath/admin/vendormanagement/VendorManagement';
function DatacenterList() {
    const navigate = useNavigate();
    const dispatch = useDispatch();
    const pagename = { pagename: 'Datacenter List' };
    ...
    const [dcname, setdcname] = useState("");  // For input value
    ...
    const fetchData = async (currentPage) => {
        try {
            const response = await connectAPIViaGet(`${dataCenterGetAll}?page=${currentPage}&page_size=${pageSize}&search=${searchTerm}`);
    ...
    const handleSubmit = (e) => {
        ...
        if (restrictedValues.includes(dcname.toLowerCase())) {
            Seterror(`"${dcname}" is not an acceptable provider name.`);
            Setsuccess('');
            return;
        }
        ...
        connectAPIViaPost(data,dataCenterSave )
            .then((response) => {
                console.log('POST request successful:', response.data);
                Setsuccess('Datacenter created successfully');
                Seterror('');
    ...
    return (
        <div className='w-[100%] h-auto min-h-screen admin-inner-body1 pb-10'>
            <div className="md:pl-[33px] pl-[20px] md:pt-[10px] pt-[10px] mb-[10px] flex">
                <p className="text-admin-color2">Vendor Management &gt; Datacenter </p>
            </div>
                ...
                <div className="admin-inner-body2 md:px-[30px] px-[10px] py-[20px] rounded-xl">
                    <main>
                        <div className='flex justify-between items-center mb-4'>
                            <div className="">
                                <h1 className="text-xl font-semibold admin-table-text text-table-text">Datacenter List </h1>
                            </div>
                            <div className='flex'>
                                <button
                                ...
                                >
                                   Datacenter List <IoMdAddCircleOutline />
                                </button>
                            </div>
                        </div>
            ...
            {isPopupOpen && (
                <div className="fixed inset-0 flex items-center justify-center md:px-0 px-3 bg-gray-600 bg-opacity-50 ">
                    <div className="bg-white p-6 rounded-lg shadow-lg">
                        <h2 className="text-xl font-semibold mb-4">Add Datacenter</h2>
                        {error && (
