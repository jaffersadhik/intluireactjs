...
import { webhookCustomerSave } from '../../../../constants/contextpath/admin/webhookmanagement/WebhookManagement';
function WebhookCustomerCreate() {
    const dispatch = useDispatch();
    const pagename = { pagename: 'Webhook Customer Create' };
    ...
    const handleSubmit = (e) => {
        ...
        connectAPIViaPost(formData,webhookCustomerSave)
            .then((response) => {
                console.log('POST request successful:', response.data);
                setSuccess('Webhook Customer  created successfully');
                setError('');
            })
            .catch((error) => {
                if (error.response) {
    ...
    return (
        <div className='w-[100%] h-auto min-h-screen admin-inner-body1 pb-10'>
            <div className="md:pl-[33px] pl-[20px] md:pt-[10px] pt-[10px] mb-[10px] flex">
                <p className="text-admin-color2">Webhook Management &gt; Webhook Customer Create</p>
            </div>
            <div className="md:px-[23px] px-[8px]">
                <div className="admin-inner-body2 md:px-[30px] px-[10px] py-[20px] rounded-xl">
                    <main>
                        <div className='flex justify-between items-center mb-4'>
                            <h1 className="text-xl font-semibold admin-table-text text-table-text">Webhook Customer Create</h1>
                        </div>
                        ...
                        <option key={customer.customer_id} value={customer.customername}>
                          {customer.customername}
                            ...
                            <div className="form-group">
                                <Button type="submit">Create Webhook Customer</Button>
                            </div>
                        </form>
...
export default WebhookCustomerCreate;
